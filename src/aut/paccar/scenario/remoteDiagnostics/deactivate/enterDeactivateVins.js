...
let enterDeactivateVins = async (vins = []) => {
    ...
    steps.push(await StepBuilder().setControl(await remoteDiagnosticsUI().fieldDeactivateVins)
    ...
    const scenario = await ScenarioBuilder().setName(`Enter Deactivate Vins: ${vins}`)
                                      .setSteps(steps)
                                      .build()
