...
/**
 * @function clickExactTableCheckbox
 * @description Click the Checkbox that contains the exact search term in the same table row
 * 
 * @param {MessageBuilder} msg
 */
const clickExactTableCheckbox = async (msg = {}) => {
  ...
  // Checkboxes are always to the most left of the table, thus columnIndex is always 0
  var rowIndex = -1
  var columnIndex = 0
  ...
  for (i = 0; i < table.length; i++) {
    for (j = 0; j < table[i].length; j++) {
      ...
      if (textElement.trim() == search.toString().trim() && rowIndex == -1) {
        rowIndex = i
        break
      }
    }
