...
/**
 * @function clickCardDeleteButton
 ...
 */
const clickCardDeleteButton = async (msg = {}) => {
  ...
  for (i = 1; i <= rowCount; i++) {
    ...
    if (rowText.includes(search)) {
      const deleteButton = await parent.findElement(By.xpath(rowPath + `//button[position() = 2]`))
      await deleteButton.click()

      result = await ResultBuilder().setChild(msg.child)
        .setStatus('pass')
        .setDetails([`Clicked Delete Button in row: ${i}`])
        .build()
      break
    }
  }
  ...
  if (i > rowCount) {
    result = await ResultBuilder().setChild(msg.child)
      .setStatus('fail')
      .setDetails([`Could not click a delete button: ${search}`])
      .build()
  }
