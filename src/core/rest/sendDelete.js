...
/**
 * @description Sends a DELETE message to a RESTful service layer.
 * @function sendDelete
 ...
 * 
 */
const sendDelete = async (url = '', body = {}) => {
  let status = 'pass'
  const details = []
  ...
  try {
    ...
    if (res.status === 200) { 
      details.push(`Successfully executed DELETE call.`)
    } else {
      status = 'fail'
      details.push(`Failed to execute DELETE, received status code: ${res.status}.`)
    }
  } catch (e) {
    status = 'fail'
    details.push(`An exception was triggered while executing a DELETE call: ${e.message} + ${e.stack}.`)
  }
