...
/**
 * @description Sends a POST message to a RESTful service layer.
 * @function sendPost
...
const sendPost = async (url = '', body = {}) => {
  ...
  try {
    ...
    if (res.status === 200) { 
      details.push(`Successfully executed POST call.`)
    } else {
      status = 'fail'
      details.push(`Failed to execute POST, received status code: ${res.status}.`)
    }
  } catch (e) {
    status = 'fail'
    details.push(`An exception was triggered while executing a POST call: ${e.message} + ${e.stack}.`)
  }
